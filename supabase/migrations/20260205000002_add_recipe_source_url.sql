...
-- Description: Stores the original URL when a recipe is imported from the web
-- Author: Claude Code
-- Date: 2026-02-05
-- =============================================================================
...
-- Add source_url column for imported recipes (NULL for manually created recipes)
ALTER TABLE public.recipes
ADD COLUMN IF NOT EXISTS source_url TEXT;

-- Partial index for recipes with source URLs (improves lookups on imported recipes)
CREATE INDEX IF NOT EXISTS idx_recipes_source_url
ON public.recipes(source_url)
WHERE source_url IS NOT NULL;

-- =============================================================================
-- 2. UPDATE CREATE_RECIPE_WITH_VERSION FUNCTION
-- =============================================================================

-- Drop existing function signature
...
-- Recreate with source_url parameter
CREATE FUNCTION public.create_recipe_with_version(
  ...
  -- NEW: Source URL for imported recipes
...
BEGIN
  ...
  -- Insert recipe with all metadata fields including source_url
...
COMMENT ON FUNCTION public.create_recipe_with_version IS
  'Creates a recipe with its initial version atomically, including all metadata fields and source_url for imported recipes';
